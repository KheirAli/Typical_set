...
def mask_to_bool(mask_img, top=None, thresh=None):
    if mask_img.ndim == 3:
        mask_img = mask_img[..., 0]
    if top is not None:
        return (mask_img == top)
    if thresh is not None:
        return (mask_img > thresh)
    return (mask_img != 0)

def rings_by_distance(mask_bool, step_px=10, n_steps=10):
    """
    mask_bool: True inside the region you want to peel inward.
    Returns list of ring boolean masks (ring 0 is the border band).
    """
    # distance inside the mask to the nearest background pixel (boundary)
    ...
    inner_after_all = dist > (len(rings) * step_px)
    return rings, inner_after_all, dist
"""
...
class mask_generator:
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None,
        ...
        """
        assert mask_type in ['box', 'random', 'both', 'extreme', 'refined_box']
    ...
    # def _retrieve_refined_box(self, img, box_coords, box_prob_multiplier=1.0, mask_path = "/home/akheirandish3/diffusion-posterior-sampling/data/mask.png"):
    #     """
    #     Create a mask with random noise where the box region has higher masking probability.
        
    #     Args:
    #         img: input image tensor
    #         box_coords: tuple (top, bottom, left, right) specifying box boundaries
    #         box_prob_multiplier: multiplier for masking probability inside the box
    #     """
    #     top, bottom, left, right = box_coords
        
    #     # Get base probability range
    #     # prob_l, prob_h = self.mask_prob_range
    #     # base_prob = np.random.uniform(prob_l, prob_h)
    #     # box_prob = min(base_prob * box_prob_multiplier, 1.0)
    #     mask = io.imread(mask_path)
    #     # Create probability map
    #     prob_map = np.full((self.image_size, self.image_size), 0.0)
    #     prob_map = 0.8 * box_prob_multiplier * (mask == top)
    #     # prob_map[top:bottom, left:right] = 0.8 # for patches uncomment this line 
        
    #     # Generate random values and create mask
    #     random_vals = np.random.random((self.image_size, self.image_size))
    #     mask_2d = (random_vals >= prob_map).astype(np.float32)
        
    #     # Convert to tensor and expand to match image shape
    #     mask_b = torch.from_numpy(mask_2d).unsqueeze(0).repeat(3, 1, 1)
    #     mask = torch.ones_like(img, device=img.device)
    #     mask[:, ...] = mask_b.to(img.device)
    ...
    def _retrieve_refined_box(self, img, box_coords, box_prob_multiplier=1.0, mask_path = "/home/akheirandish3/diffusion-posterior-sampling/data/mask.png"):
        ...
        Args:
            ...
            box_prob_multiplier: multiplier for masking probability inside the box
        """
        top, num_rings, left, right = box_coords
        ...
        base = mask_to_bool(mask, top=top)   # or thresh=127, or default nonzero

        rings, inner, dist = rings_by_distance(base, step_px=10, n_steps=10)

        prob_map = np.full((self.image_size, self.image_size), 0.0)
        # prob_map = box_prob_multiplier * (mask == top)
        # prob_map = box_prob_multiplier * ((mask == top) | (mask == (top + 15))) ## NEW
        prob_map = box_prob_multiplier * ((mask == (top + 150))) ## NEW
    # deterministic ring-missing mask

        mask_2d = np.ones((256, 256), dtype=np.float32)
        if num_rings == 3:
            mask_2d = (mask_2d - (1.0*(mask == top)).astype(np.float32))
        elif num_rings == 4:
            random_vals = np.random.random((self.image_size, self.image_size))
            mask_2d = (random_vals >= prob_map).astype(np.float32)
        else:
            mask_2d[rings[num_rings]] = 0.0
    ...
    def __call__(self, img, box_coords=None, box_prob_multiplier=1.0, mask_path=None):
        if self.mask_type == 'random':
            ...
            mask = 1. - mask
            return mask
        elif self.mask_type == 'refined_box':
            assert box_coords is not None, "box_coords required for refined_box mask type"
            if mask_path is None:

                mask_path = "/home/akheirandish3/diffusion-posterior-sampling/data/mask.png"
            mask = self._retrieve_refined_box(img, box_coords, box_prob_multiplier=box_prob_multiplier, mask_path=mask_path)
            # mask = self._retrieve_refined_box(img, box_coords)
            return mask
